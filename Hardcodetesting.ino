...
float testarray[1500]=;//hardcode here;
float irarray[1500];           // raw IR values
int arraysize[1500];           // index array
float heartbeattime[1500];
unsigned long timearray[1500]; // timestamps
unsigned long P2PTime_Array[1500]; // This is for the p2p time
...
void loop() {
  ...
  if (testarray[i]<=1500) {
    ...
    if (checkForBeat(ir) == true) {
      long delta = 60/100;
      lastBeat = 0;
      P2PTime_Array[p] = delta; // seconds
      p++;
      beatsPerMinute = 60 / (delta);
