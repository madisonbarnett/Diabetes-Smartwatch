...
#define RST_PIN 32
#define ADCPIN A7
#define SHAKE_PIN 37   // Shake switch pin
...
uint16_t menuColors[] = {TFT_PINK, TFT_SKYBLUE, TFT_VIOLET, TFT_ORANGE}; // maybe change orange later?
...
int graphCount; // changed this to not reset to 0, may need to be changed back
...
const unsigned long SLEEP_TIMEOUT = 45000; // 60 seconds
...
void writeHistoryData(float predictedGlucose) {
  File file = LittleFS.open("/history.txt", "a"); // "a" appends, "w" overwrites
  ...
  line += timestamp;
  line += ",";
  line += String(predictedGlucose, 2); // rounds to 2 decimal places

  // Save features - REMOVE IF MEMORY IS A PROBLEM
  // Can also decrease decimal values for features to save memory
  line += "," + String(values[0]); // Age
  line += "," + String(values[3] * 0.453592f); // Weight in kg
  line += "," + String(values[2] * 2.54f); // Height in cm
  line += "," + String(values[4]); // Diagnosis
...
void summarizeHistoryData() {
  ...
  while (file.available()) {
    ...
    // Convert to struct to use mktime to create epoch (seconds since Jan 1, 1970) for easier comparison
    struct tm t;
    t.tm_year = year - 1900;
    t.tm_mon  = month - 1;
    t.tm_mday = day;
...
void loadHistoryGraph72h() {
  ...
  struct tm nowTm;
  nowTm.tm_year = rtc.getYear() - 1900;
  nowTm.tm_mon  = rtc.getMonth() - 1; // double check this
  nowTm.tm_mday = rtc.getDay();
  nowTm.tm_hour = rtc.getHour();
  nowTm.tm_min  = rtc.getMinute();
  nowTm.tm_sec  = rtc.getSecond();
  nowTm.tm_isdst = -1;
...
void displayHistoryGraph() {
  ...
  // Centered graph region on 240x240 screen
  const int plotW = 140;
  const int plotH = 110;
  const int plotX = ((240 - plotW) / 2) + 5;   // 50
  const int plotY = 55;
  ...
  // Y labels to the left of the graph, vertically separated
  tft.setTextDatum(MR_DATUM);
  tft.drawString(String(maxVal, 1), plotX - 4, plotY + 4);
  tft.drawString(String(minVal, 1), plotX - 4, plotY + plotH - 4);

  // X labels below the graph, farther down so they don't collide
...
void enableMAX30102() {
  ...
  if (!particleSensor.begin(I2C_2, I2C_SPEED_STANDARD)) {
    ...
    return;  // ← sampling will run but collect 0 samples
  }
  // Explicit 100 Hz, no sample averaging — fixes low sample count
...
// Runs inference using current feature globals and profile values.
// Input order must match training pipeline exactly.
float runInference() {
  ...
  tfl_input->data.int8[0]  =(int8_t)(((((float)values[0]-standardmean[0])/standardscalar[0])/ in_scale)+in_zero_point);  // Age repplace the float number with the global variable in sample
  tfl_input->data.int8[1]  =(int8_t)(((((float)values[3]*0.45359237-standardmean[1])/standardscalar[1])/in_scale)+in_zero_point);  // Weight cm
  tfl_input->data.int8[2]  =(int8_t)(((((float)values[2]*2.54-standardmean[2])/standardscalar[2])/in_scale)+in_zero_point);  // Height cm
  tfl_input->data.int8[3]  =(int8_t)(((((float)values[4]-standardmean[3])/standardscalar[3])/in_scale)+in_zero_point);  // PreOpDm
  tfl_input->data.int8[4]  =(int8_t)((((meanP2Pinterval-standardmean[4])/standardscalar[4])/in_scale)+in_zero_point);  // PPGMEANINTERVAL
  tfl_input->data.int8[5]  =(int8_t)((((stddev -standardmean[5])/standardscalar[5])/in_scale)+in_zero_point);  // PPGSTD
  tfl_input->data.int8[6]  =(int8_t)((((teager_max-standardmean[6])/standardscalar[6])/in_scale)+in_zero_point);  // PPGTEAGER
  tfl_input->data.int8[7]  =(int8_t)((((skewness-standardmean[7])/standardscalar[7])/in_scale)+in_zero_point);  // PPGSKEW
  tfl_input->data.int8[8]  =(int8_t)((((iqr-standardmean[8])/standardscalar[8])/in_scale)+in_zero_point);  // PPGIQR
  tfl_input->data.int8[9]  =(int8_t)((((entropy-standardmean[9])/standardscalar[9])/in_scale)+in_zero_point);  // PPGENTROPY
  tfl_input->data.int8[10] =(int8_t)((((FIRST_DERIVMAX -standardmean[10])/standardscalar[10])/in_scale)+in_zero_point); //first deriv max
  tfl_input->data.int8[11] = (int8_t)((((stddevTime-standardmean[11])/standardscalar[11])/in_scale)+in_zero_point);// PPGSTDPP
...
void updateSamplingProgress() {
  ...
  if (elapsedMs > SAMPLE_DURATION_MS) elapsedMs = SAMPLE_DURATION_MS;

  int pct = (elapsedMs * 100UL) / SAMPLE_DURATION_MS;
  int fillW = ((barW - 4) * pct) / 100;

  if (fillW != lastFillW) {
    if (lastFillW > 0) {
      tft.fillRect(barX + 2, barY + 2, lastFillW, barH - 4, TFT_BLACK);
    }
...
bool runSampling() {
  ...
  while ((millis() - initial) < SAMPLE_DURATION_MS && i < 1500) {
...
void drawMenuIcon(int idx, int cx, int cy, int size, uint16_t color) {
  ...
  else if (idx == 3) { // this needs to be tested
    int rOuter = size / 3;
    int rInner = rOuter - 6;

    // Thick clock ring
    tft.fillCircle(cx, cy, rOuter, color);
    tft.fillCircle(cx, cy, rInner, TFT_ORANGE); // change this if the orange changes
...
void displaySampleMenu() {
  ...
  int startX = cx - 15;
  int endX   = cx + 15;
  // Arrow pointing right / toward sensor
  tft.drawLine(startX, cy, endX, cy, TFT_WHITE);
  tft.drawLine(endX, cy, endX - 12, cy - 12, TFT_WHITE);
  tft.drawLine(endX, cy, endX - 12, cy + 12, TFT_WHITE);

  // 3 second pause for user to move finger but allows exit
...
void displayHistoryMenu() {
  ...
  if (historyCount == 0) {
    ...
    //tft.setTextSize(2);
    //tft.drawString("History", tft.width()/2, 80);
...
void setup() {
  Serial.begin(115200);
  delay(500); // longer delay to make sure serial is ready
  print_wakeup_reason();
  ...
  if (currentScreen == HOME_SCREEN) {
    ...
    if (btnPlus.justReleased()) {
      ...
      displaySettingsMenu();
          ...
          if (historyView == 0 && btnBack.justReleased()) {
            drawMenuCarousel();
            return;
          }

          if (historyView == 1 && btnBack.justReleased()) {
            drawMenuCarousel();
            return;
          }

          if (historyView == 2 && btnBack.justReleased()) {
            drawMenuCarousel();
            return;
          }
