...
# Toggle switches to determine what to plot
HISTOGRAMS = False
HEATMAPS = True

# Load data into dataframe
bg_df = pd.read_csv('processed_data/delta_vitaldb_ppg_extracted_features_15s_5minwin.csv')
...
if HEATMAPS == True:
    ...
    bg_df = bg_df.drop('ppg_std', axis=1)
    bg_df = bg_df.drop('ppg_mean', axis=1)
    ...
    bg_df = bg_df.drop('pwm_rise_time', axis=1)
    bg_df = bg_df.drop('pwm_decay_time', axis=1)
    bg_df = bg_df.drop('pwm_max_slope', axis=1)
    bg_df = bg_df.drop('pwm_min_slope', axis=1)
    bg_df = bg_df.drop('delta_ppg_freq', axis=1)
