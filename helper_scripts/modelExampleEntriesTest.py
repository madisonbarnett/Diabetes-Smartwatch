import re
import time
from pathlib import Path

import joblib
import numpy as np
import pandas as pd
import tensorflow as tf
from sklearn.metrics import mean_absolute_percentage_error, r2_score

from cega import cega

MODELPATH = Path("model_weights/mlp.keras")
SCALERPATH = Path("model_weights/mlp_scalers.pkl")
TFLITE_PATH = Path("model_weights/mlp_int8.tflite")
C_HEADER_PATH = Path("model_weights/mlp.h")
EXAMPLE_ROWS_PATH = Path("vitaldb_20_example_rows.csv")
OUTPUT_PATH = Path("example_entries_predictions_comparison.csv")
TARGET_COLUMN = "gluc"
FEATURE_COLUMNS = [
    "age",
    "weight",
    "height",
    "preop_dm",
    "ppg_mean_pp_interval_s",
    "ppg_std",
    "ppg_teager_energy",
    "ppg_skew",
    "ppg_iqr",
    "ppg_entropy",
    "ppg_first_deriv_max",
    "ppg_std_pp_interval_s",
]


def run_keras_inference(model, x_test, scaler=None):
    if scaler is not None:
        x_test = scaler.transform(x_test)
    return model.predict(x_test, batch_size=2048, verbose=0).flatten()


def run_tflite_inference(tflite_path, x_test, scaler=None):
    print(f"Loading TFLite model: {tflite_path}")
    interpreter = tf.lite.Interpreter(model_path=str(tflite_path))
    interpreter.allocate_tensors()

    input_details = interpreter.get_input_details()[0]
    output_details = interpreter.get_output_details()[0]

    input_scale, input_zero_point = input_details["quantization"]
    output_scale, output_zero_point = output_details["quantization"]

    is_input_int8 = input_details["dtype"] == np.int8
    is_output_int8 = output_details["dtype"] == np.int8

    predictions = []

    print("Running TFLite inference (int8)...")
    start = time.time()

    for i in range(len(x_test)):
        x = x_test[i : i + 1]

        if scaler is not None:
            x = scaler.transform(x)

        if is_input_int8:
            x = (x / input_scale + input_zero_point).astype(np.int8)

        interpreter.set_tensor(input_details["index"], x)
        interpreter.invoke()

        y = interpreter.get_tensor(output_details["index"])[0]

        if is_output_int8:
            y = (y.astype(np.float32) - output_zero_point) * output_scale

        predictions.append(y)

    tflite_time = time.time() - start
    print(f"TFLite inference finished in {tflite_time:.2f} seconds")

    return np.array(predictions).flatten()


def run_c_header_inference(header_path, x_test, scaler=None):
    print(f"Loading model from C header: {header_path}")

    with open(header_path, "r", encoding="utf-8") as file:
        content = file.read()

    match = re.search(r"=\s*\{(.*?)\}\s*;", content, re.DOTALL)
    if not match:
        raise ValueError(
            f"Could not find the byte array in {header_path}. "
            "Make sure the file contains 'unsigned char ...[] = {'"
        )

    byte_str = match.group(1)
    hex_values = re.findall(r"0x[0-9a-fA-F]+", byte_str)
    if not hex_values:
        raise ValueError("No hex values (0x..) found in the C header file.")

    print(f"Successfully extracted {len(hex_values):,} bytes from mlp.h")
    model_bytes = np.array([int(value, 16) for value in hex_values], dtype=np.uint8)
    print(f"Model byte array created: {len(model_bytes):,} bytes")

    interpreter = tf.lite.Interpreter(model_content=model_bytes.tobytes())
    interpreter.allocate_tensors()

    input_details = interpreter.get_input_details()[0]
    output_details = interpreter.get_output_details()[0]

    input_scale, input_zero_point = input_details["quantization"]
    output_scale, output_zero_point = output_details["quantization"]

    is_input_int8 = input_details["dtype"] == np.int8
    is_output_int8 = output_details["dtype"] == np.int8

    predictions = []
    print("Running inference from C header (int8)...")
    start = time.time()

    for i in range(len(x_test)):
        x = x_test[i : i + 1].copy()

        if scaler is not None:
            x = scaler.transform(x)

        if is_input_int8:
            x = (x / input_scale + input_zero_point).astype(np.int8)

        interpreter.set_tensor(input_details["index"], x)
        interpreter.invoke()

        y = interpreter.get_tensor(output_details["index"])[0]

        if is_output_int8:
            y = (y.astype(np.float32) - output_zero_point) * output_scale
        else:
            y = y.astype(np.float32)

        predictions.append(y[0] if isinstance(y, np.ndarray) and y.ndim > 0 else float(y))

    c_time = time.time() - start
    print(
        f"C header inference finished in {c_time:.2f} seconds "
        f"({c_time / len(x_test) * 1000:.2f} ms per sample)"
    )

    return np.array(predictions).flatten()


def print_metrics(name, y_actual, y_pred):
    r2 = r2_score(y_actual, y_pred)
    mae = np.mean(np.abs(y_actual - y_pred))
    mape = mean_absolute_percentage_error(y_actual, y_pred) * 100
    print(f"\n{name} Results:")
    print(f"    R²   : {r2:.3f}")
    print(f"    MAE  : {mae:.2f} mg/dL")
    print(f"    MAPE : {mape:.2f}%")
    cega(y_actual, y_pred)


def main():
    print("Loading scaler and example entries...")
    scaler = joblib.load(SCALERPATH)
    df_examples = pd.read_csv(EXAMPLE_ROWS_PATH, dtype="float32")

    required_columns = [TARGET_COLUMN, *FEATURE_COLUMNS]
    missing_columns = [column for column in required_columns if column not in df_examples.columns]
    if missing_columns:
        raise ValueError(
            f"Missing required columns in {EXAMPLE_ROWS_PATH}: {missing_columns}"
        )

    x_test = df_examples[FEATURE_COLUMNS].values.astype(np.float32)
    y_actual = df_examples[TARGET_COLUMN].values.astype(np.float32)
    entry_ids = np.arange(1, len(df_examples) + 1)

    print("\n=== Testing Keras (float32) model ===")
    model = tf.keras.models.load_model(MODELPATH)
    y_pred_keras = run_keras_inference(model, x_test, scaler)

    print("\n=== Testing int8 TFLite model ===")
    y_pred_tflite = run_tflite_inference(TFLITE_PATH, x_test, scaler)

    print("\n=== Testing final C header (mlp.h) ===")
    y_pred_c = run_c_header_inference(C_HEADER_PATH, x_test, scaler)

    results = pd.DataFrame(
        {
            "example_row": entry_ids,
            "actual_gluc": y_actual,
            "predicted_keras": np.round(y_pred_keras, 4),
            "predicted_tflite_int8": np.round(y_pred_tflite, 4),
            "predicted_c_header": np.round(y_pred_c, 4),
            "error_keras": np.abs(y_actual - y_pred_keras),
            "error_tflite": np.abs(y_actual - y_pred_tflite),
            "error_c_header": np.abs(y_actual - y_pred_c),
        }
    )

    results.to_csv(OUTPUT_PATH, index=False)
    print(f"\nFull comparison saved to '{OUTPUT_PATH}'")

    print_metrics("Keras (float32)", y_actual, y_pred_keras)
    print_metrics("TFLite (int8)", y_actual, y_pred_tflite)
    print_metrics("C Header (mlp.h)", y_actual, y_pred_c)

    print("\n" + "=" * 70)
    print(f"You can now open '{OUTPUT_PATH}'")
    print("to compare actual vs predicted values across all three versions.")
    print("=" * 70)


if __name__ == "__main__":
    main()
