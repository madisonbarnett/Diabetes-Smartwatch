...
*  The code features a GUI with four screens: profile, sample, settings, and history. The user is intended to enter biographic data into the profile menu
*  which the machine learning model, mlp.h, uses in combination with features from a MAX30102 PPG sensor to predict the user's blood glucose value.
*
*  The mlp.h machine learning model was trained on data from: https://vitaldb.net/dataset/
*  
*  A Waveshare 1.28 in. round LCD display with touch interface (CST816 controller) is used. It uses I2C for touch input and SPI for screen control.
*  A MAX30102 pulse oximeter is used as well, which uses I2C and some libraries as seen below.
*  See the final documentation for a full formal parts list.
*/

// Screen
#include <Wire.h>
#include <TFT_eSPI.h> // Library for display --> UserSetup.h in library file needs to be set up with assigned pins & GC9A01 Driver
...
#define INT_PIN 38
#define RST_PIN 32
#define ADCPIN A7 // Pin for voltage divider - unused in V1 Prototype
#define SHAKE_PIN 37
...
ESP32Time rtc(0); // Offset for time
...
int values[] = {21, 0, 65, 130, 1}; // Default values
int fieldIndex = 0;
...
const unsigned long SLEEP_TIMEOUT = 45000; // Goes to sleep after 45 seconds of no touch
...
// When profile data is changed, write to file
...
// Read previously saved profile data
...
// Clears history samples after 2 weeks
void pruneHistoryData() {
  File fileIn = LittleFS.open("/history.txt", "r");
  if (!fileIn) return;

  File fileOut = LittleFS.open("/history_tmp.txt", "w");
  if (!fileOut) { fileIn.close(); return; }

  // Build cutoff epoch from current RTC time
  String nowStr = rtc.getTime("%Y-%m-%d %H:%M:%S");
  struct tm nowTm = {};
  nowTm.tm_year = nowStr.substring(0, 4).toInt() - 1900;
  nowTm.tm_mon  = nowStr.substring(5, 7).toInt() - 1;
  nowTm.tm_mday = nowStr.substring(8, 10).toInt();
  nowTm.tm_hour = nowStr.substring(11, 13).toInt();
  nowTm.tm_min  = nowStr.substring(14, 16).toInt();
  nowTm.tm_sec  = nowStr.substring(17, 19).toInt();
  nowTm.tm_isdst = -1;
  unsigned long cutoff = mktime(&nowTm) - (14UL * 86400UL);  // 14 days ago
  ...
  LittleFS.rename("/history_tmp.txt", "/history.txt");

  Serial.print("History pruned, entries kept: ");
  Serial.println(kept);
}

// Writes each sample to history.txt
...
// Parses history.txt and finds average, min, max, and timespan
void summarizeHistoryData() {
  ...
  while (file.available()) {
    ...
    // Converts to epoch for easy comparison
    struct tm t;
    t.tm_year = year - 1900;
    t.tm_mon  = month - 1;
    t.tm_mday = day;
...
// Parses history.txt to find data from previous 72 hours for graphing
void loadHistoryGraph72h() {
  File file = LittleFS.open("/history.txt", "r");
  if (!file) {
    graphCount = 0;
...
// Draws the history graph - displays max value from past 72 hours as Y limit and min value as Y min; draws dots for each point and lines to connect them
void displayHistoryGraph() {
  ...
  const int plotW = 140;
  const int plotH = 110;
  const int plotX = ((240 - plotW) / 2) + 10;
  const int plotY = 55;
...
// Function to manually clear all history data
void clearHistoryData() {
...
// Function to display the button to clear history data on the third carousel screen of the history menu
void displayHistoryClearMenu() {
  currentScreen = HISTORY_SCREEN;
  historyView = 2;
  tft.fillScreen(TFT_BLACK);
...
// ================= SLEEP =================
// Current code uses tap on screen to wake device from deep sleep, but if shake switch is being used, call this function in setup()
void pinsetup() {
  pinMode((int)SHAKE_PIN, INPUT_PULLDOWN);
}

// Debug function to determine why device woke up 
...
// Saves current RTC time to file before each deep sleep to reset clock to previous time when power is disconnected and re-connected
void saveTimeToFile() {
  File file = LittleFS.open("/lasttime.txt", "w");
  if (!file) { Serial.println("Failed to save time"); return; }
  file.println(rtc.getTime("%Y-%m-%d %H:%M:%S"));
  file.close();
  Serial.println("Time saved before sleep");
}

// Checks if user has touched screen in last 45 seconds, saves time, and enters deep sleep if not
void sleep_param() {
  ...
  digitalWrite(25, LOW); // Turns screen off
  delay(100);
...
// Function to turn sensor on upon entering sampling menu to preserve battery
void enableMAX30102() {
  if (max30102_active) return;
  I2C_2.begin(7, 8); // SDA and SCL pins for MAX30102
...
// Function to turn sensor off to preserve battery
...
// ================= FEATURE FUNCTIONS =================
// Following functions all find various features from the PPG signal
...
// Function to exit sampling loop
bool sampleBackPressed() {
...
// Two following functions draw the progress bar on a 15 second timer - if sampling time changes this needs to be manually updated to new time
void drawSamplingUI() {
  tft.fillScreen(TFT_BLACK);
...
// Function to draw back arrow used throughout menus
...
// Function to draw each icon on the menu carousel
...
void displayHome() {
  ...
  tft.fillScreen(TFT_BLACK);
  btnUnlock.initButton(&tft, tft.width()/2, tft.height()/2, tft.width(), tft.height(), TFT_BLACK, TFT_BLACK, TFT_BLACK, "", 1);
  tft.setTextDatum(MC_DATUM);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
...
void displayProfileMenu() {
  ...
  // Buttons to increase and decrease field value
  ...
  // Back button, doesn't save any changes
  btnBack.initButton(&tft, tft.width()/2, 30, 50, 30, TFT_BLACK, TFT_BLACK, TFT_WHITE, "", 1);
  btnBack.drawButton();
  drawBackArrow(tft.width()/2, 30);
...
void displaySampleMenu() {
  ...
  // Allows exit from sampling loop by periodically polling button
...
void displaySettingsMenu() {
  ...
  // Buttons to change value in field
  btnMinus.initButton(&tft, 45, 130, 50, 45, TFT_WHITE, TFT_BLACK, TFT_WHITE, "-", 3);
  btnPlus.initButton(&tft, 195, 130, 50, 45, TFT_WHITE, TFT_BLACK, TFT_WHITE, "+", 3);
  btnMinus.drawButton(); 
  btnPlus.drawButton();
...
void displayHistoryMenu() {
  currentScreen = HISTORY_SCREEN;
  historyView = 0; // 0 for summary screen
...
void setup() {
  ...
  // If using tilt ball switch for wake-up:
  // Hold-off: if we just woke from tilt, wait for switch to settle before
  // the sleep timer can start counting, preventing instant re-sleep
  delay(200);
  lastTouch = millis();   
  ...
  //pinsetup(); // Enable this and next line if using tilt ball switch for wake up
  //esp_sleep_enable_ext1_wakeup((1ULL << SHAKE_PIN), ESP_EXT1_WAKEUP_ANY_HIGH);

  // Disable these lines if using tilt ball for wake up - enable for touch to wakeup
  rtc_gpio_pullup_en((gpio_num_t)INT_PIN);
  rtc_gpio_pulldown_dis((gpio_num_t)INT_PIN);
  esp_sleep_enable_ext0_wakeup((gpio_num_t)INT_PIN, 0);  // wake on LOW (INT_PIN is active-low)
  ...
  // Checks if previous time was saved before power was disconnected - if so, sets time to last saved
  if (LittleFS.exists("/lasttime.txt")) {
  File file = LittleFS.open("/lasttime.txt", "r");
  if (file) {
    ...
    if (ts.length() >= 19) {
      ...
      rtc.setTime(sc, mn, hr, day, mo, yr);
      Serial.println("Time restored from file: " + ts);
    }
  }
} else {
  // First ever boot, no saved time - set to default
  rtc.setTime(0, 52, 2, 18, 4, 2026);
}
...
void loop() {
  ...
  if (touch.available()) {
    ...
    // Following if loops enable the specific buttons on each screen based on when screen is activated
  ...
  // Disables all buttons if no screen active
  else {
  ...
  // === ACTIONS BY SCREEN ===
  if (currentScreen == HOME_SCREEN) {
    if (btnUnlock.justReleased()) 
      drawMenuCarousel();
  } 
  else if (currentScreen == MENU_SCREEN) {
    if (btnBack.justReleased()) {
      // Since back button is always in same spot, this prevents it being pressed on multiple screens at once
    ...
    if (btnConfirm.justReleased()) { 
      writeProfileData(); // If changes are saved, writes new profile data to file
      drawMenuCarousel(); 
    }
    ...
    if (btnConfirm.justReleased()) {
      // If saved, updates RTC time
      rtc.setTime(0, settings_values[1], settings_values[0], settings_values[3], settings_values[2], settings_values[4]);
      settings_loaded = false;
      drawMenuCarousel();
    }
    if (btnPrevField.justReleased()) {
    ...
    if ((gest == "SWIPE LEFT" || gest == "SWIPE RIGHT") &&
      ...
      if (gest == "SWIPE LEFT") {
        // Displays menu based on history view selected
    ...
    if (historyView == 2 && btnConfirm.justReleased()) {
      clearHistoryData(); // Deletes history data 
      displayHistoryMenu();
      return;
    }
  }

  sleep_param(); // Checks when last touch was and goes to sleep if past 45 seconds
  delay(15);
}
