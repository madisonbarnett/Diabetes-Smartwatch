//PCB version: pins updated and battery estimation added
//Note: TFT_eSPI User_Setup.h pins must be set up correctly
...
int settings_values[] = { 4, 0, 4, 11, 2026 };
...
void writeHistoryData(float predictedGlucose) {
  ...
  if (!file) {
    Serial.println("Failed to open history file for appending");
    return;
  }
...
void displayHistoryGraph() {
  ...
  for (int i = 0; i < graphCount - 1; i++) {
    ...
    int y1 = plotY + plotH - 1 - (int)(((graphGlucose[i] - minVal) / (maxVal - minVal)) * (plotH - 1));
    int y2 = plotY + plotH - 1 - (int)(((graphGlucose[i + 1] - minVal) / (maxVal - minVal)) * (plotH - 1));
  ...
  int lastY = plotY + plotH - 1 - (int)(((graphGlucose[graphCount - 1] - minVal) / (maxVal - minVal)) * (plotH - 1));
...
void displayBattery() {
  pinMode(Q_GATE, OUTPUT);
  digitalWrite(Q_GATE, HIGH);
  delay(50);
  batteryValue = analogRead(ADCPIN);
  voltValue = ((batteryValue * 3.3f) / 4095.0f);
  estimation = ((voltValue - 3.0f) / 1.2f) * 100.0f;
  if (estimation > 100) {
    estimation = 100;
...
void setup() {
  ...
  pinMode(LCD_VIN, OUTPUT);
  digitalWrite(LCD_VIN, HIGH);
  pinMode(LCD_GND, OUTPUT);
  digitalWrite(LCD_GND, LOW);
  pinMode(PPG_VIN, OUTPUT);
  digitalWrite(PPG_VIN, HIGH);
  pinMode(PPG_GND, OUTPUT);
  digitalWrite(PPG_GND, LOW);
...
void loop() {
  ...
  if (currentScreen == HOME_SCREEN) {
    ...
    if ((gest == "SWIPE LEFT" || gest == "SWIPE RIGHT") && (now - lastGestureMs > GESTURE_LOCKOUT_MS)) {
